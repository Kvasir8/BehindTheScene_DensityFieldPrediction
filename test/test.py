...
class DensityFieldTransformer(nn.Module):
    def __init__(self, d_model=103, att_feat=103, nhead=1, num_layers=4, feat_pad=True, DFEnlayer=True, AE=True, do_=0.,
                 rb_=4096, ren_nc=64, B_=16):  ## dim_feedforward==input_feature Map_spatial_flattened_dim
        """
        ...
        self.ts_ = B_ * ren_nc * (8 * 8) * (
                    rb_ // (8 * 8))  ## total num sampled points (to decide input dimension for AE)
        ...
        if self.DFEnlayer:
            ...
            self.transformer_encoder = mlp.TrEnLayer(self.transformer_enlayer,
                                                     num_layers)  ## TODO: replace MHA module with IBRNet network and complete integratable encoder part of transformer
        else:
            self.transformer_enlayer = TransformerEncoderLayer(att_feat, nhead, dim_feedforward=att_feat,
                                                               batch_first=True)
            self.transformer_encoder = TransformerEncoder(self.transformer_enlayer, num_layers)

        self.readout_token = nn.Parameter(torch.rand(1, 1, att_feat).to("cuda"),
                                          requires_grad=True)  ## ? # self.readout_token = torch.rand(1, 1, d_model).to("cuda") ## instead of dummy
        ...
        if self.AE:
            ...
            self.density_field_prediction = mlp.ConvAutoEncoder(self.att_feat,
                                                                self.S_)  ## [1, 2*self.att_feat, self.ts_] ## self.att_feat*2 ## self.ts_ ##(patch_size x ray_batch_size) self.att_feat, sampled_features.shape[0] or nv_+1 == 5 TODO: investigate more the model sctructure for validity in detail
        else:
            self.density_field_prediction = nn.Sequential(nn.Linear(self.att_feat,
                                                                    1))  ## Note: ReLU or Sigmoid would be detrimental for gradient flow at zero center activation function
    ...
    def forward(self, sampled_features, invalid_features):  ### [n_, nv_, M, C1+C_pos_emb], [nv_==2, M==100000, C==1]
        ...
        invalid_features = (
                    invalid_features > 0.5)  ## round the each of values of 3D points simply by step function within the range of std_var [0,1]
        assert invalid_features.dtype == torch.bool, f"The elements of the {invalid_features} are not boolean."

        if self.dropout:  invalid_features = 1 - self.dropout((
                                                                          1 - invalid_features.float()))  ## TODO: after dropping out, the values of elements are 2 somehow why?? ## randomly zero out the valid sampled_features' matrix. i.e. (1-invalid_features)

        encoded_features = self.emb_encoder(sampled_features.flatten(0, -2)).reshape(
            sampled_features.shape[:-1] + (-1,))  ### [M*n==100000, nv_==6, 32]   ## Embedding to Transformer arch.

        ## Process the embedded features with the Transformer
        if self.padding_flag:
            padded_features = torch.concat(
                [self.readout_token.expand(encoded_features.shape[0], -1, -1), encoded_features],
                dim=1)  ### (B*n_pts, nv_+1, 103) == ([100000, 2+1, 103]): padding along the column ## Note: needs to be fixed for nicer way
            padded_invalid = torch.concat([torch.zeros(invalid_features.shape[0], 1, device="cuda"), invalid_features],
                                          dim=1, )
            # invalid_features[...,0].permute(1,0) ### [M, num_features + one zero padding layer] == [6250, 96+1]
            if self.DFEnlayer:
                transformed_features = self.transformer_encoder(padded_features,
                                                                src_key_padding_mask=padded_invalid)  ### masking dim(features) ([100000 * B, 1+nv_, 103]) with invalid padding [100000, 3])    ## self.transformer_enlayer for one encoder layer
            else:
                transformed_features = self.transformer_encoder(padded_features,
                                                                src_key_padding_mask=padded_invalid)  ### masking dim(features) ([100000 * B, 1+nv_, 103]) with invalid padding [100000, 3])
            ...
            transformed_features = self.transformer_encoder(encoded_features,
                                                            src_key_padding_mask=invalid_features)  ### [100000, nv_==2, 103]

        aggregated_features = transformed_features[:, 0,
                              :]  ### [M_b, nv_+1 ,att_feat==32]  ## first token refers to the readout/view-independent/global-geometric understanding token where it stores the feature information accumulated from the layers    # aggregated_features = self.attention(self.query.expand(transformed_features.shape[0], -1, -1), transformed_features, transformed_features, key_padding_mask=invalid_features)[0]
        ...
        if self.AE:
            aggregated_features = aggregated_features.permute(-1, 0).unsqueeze(
                0)  ### (C,==att_feat)  ## shrink the density fields to fit into the AE layer by making them average pooling
            aggregated_features = self.ConvNet2AE(aggregated_features,
                                                  self.S_)  ## .permute(0,-1,1).squeeze(-1) ### (1,32,10240)
