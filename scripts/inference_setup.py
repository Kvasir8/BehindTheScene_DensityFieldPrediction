...
def render_poses(renderer, ray_sampler, poses, projs, black_invalid=False):
    ...
    invalid = (
        render_dict["coarse"]["invalid"].squeeze(-1) * render_dict["coarse"]["weights"]
    ).sum(-1).squeeze() > 0.8
...
def render_profile(net, cam_incl_adjust, viewdirs=None):
    ...
    if q_pts.shape[1] > batch_size:
        ...
        for i in range(math.ceil(l / batch_size)):
            ...
            _, invalid_, sigmas_, _ = net.forward(
                q_pts_, viewdirs=viewdirs
            )  ## default, note: viewdirs is for neuray
