...
class IBRNetRenderingWrapper(nn.Module):
    ...
    def convert_camera(self, Ks, poses_c2w, ids, height, width):
        padded_Ks = torch.nn.functional.pad(Ks, (0, 1, 0, 1), mode="constant", value=0.0)
    ...
    def convert_output(self, raw, pixel_mask):
        ...
        return rgb, invalids, sigma

    def compute_grid_transforms(self, *args, **kwargs):
        pass

    def encode(self, images, Ks, poses_c2w, ids_encoder=None, ids_render=None):
        ids_encoder = [1, 2, 3]
        ...
        self.src_cameras = self.convert_camera(Ks, poses_c2w, ids_encoder, height, width)
        self.que_camera = self.convert_camera(Ks, poses_c2w, ids_render, height, width).flatten(0, 1)

    def forward(self, xyz, coarse=True, viewdirs=None):
        xyz = xyz.reshape(-1, 64, 3)
        rgb_feat, ray_diff, mask = self.projector.compute(
            xyz, self.que_camera, self.src_imgs, self.src_cameras, featmaps=self.featmaps[0]
        )
