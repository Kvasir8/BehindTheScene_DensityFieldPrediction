...
class DensityFieldTransformer(nn.Module):
    def __init__(self, d_model=103, att_feat=32, nhead=1, num_layers=4, feat_pad=True, DFEnlayer=True,
        ...
        if self.DFEnlayer:
            self.transformer_enlayer = mlp.EncoderLayer(att_feat, att_feat, nhead, att_feat // nhead, att_feat // nhead)
        ...
        self.readout_token = nn.Parameter(torch.rand(1, 1, att_feat).to("cuda"),requires_grad=True)
