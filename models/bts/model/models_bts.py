...
class MVBTSNet(torch.nn.Module):
    ...
    def encode(self, images, Ks, poses_c2w, ids_encoder=None, ids_render=None, images_alt=None, combine_ids=None):  ## ids_encoder:=which img to begin with
        ...
        if self.nry:
            self.img_feat = F.interpolate(img_feat_ms[-1], size=(h_, w_)).view(n_, nv_, img_feat_ms[-1].shape[1], h_, w_)    ## upsampling the feature maps from down-sampled 4 layers to the same spatial resolution of 1st layer

        # self.img_feat = image_latents_ms[-1]      ## taking last layer of encoder to feed into Neuray
        # self.grid_f_img_feat = img_feat_ms                    ## to feed into NeuRay's img feat
    ...
    def sample_features(self, xyz, use_single_featuremap=True):     ## 2nd arg: to control whether multiple feature maps should be combined into a single feature map or not. If True, the function will average the sampled features from multiple feature maps along the view dimension (nv) before returning the result. This can be useful when you want to combine information from multiple views or feature maps into a single representation.
        ...
        if self.code_mode == "z":                                                           ## Depending on the code mode, normalize the depth value or distance value to the [-1, 1] range and concatenate with the xy coordinates
            ...
            if self.inv_z:  z_ = (1 / z_.clamp_min(EPS) - 1 / self.d_max) / (1 / self.d_min - 1 / self.d_max)
            else:           z_ = (z_ - self.d_min) / (self.d_max - self.d_min)
            z_ = 2 * z_ - 1
        ...
        if self.nry:
            self.img_feat = self.img_feat[:, :nv_]                                             ## taking last layer of encoder to feed into NeuRay
            sampled_img_feat = F.grid_sample(self.img_feat.view(n_ * nv_, self.img_feat.shape[2], h_, w_), xy.view(n_ * nv_, 1, -1, 2), mode="bilinear", padding_mode="border", align_corners=False).view(n_, nv_, self.img_feat.shape[2], n_pts).permute(0, 3, 1, 2)
        else: sampled_img_feat = None       ### dim(sampled_features): (n_, nv_, n_pts, c_)
        ...
        return sampled_features, invalid[..., 0].permute(0, 2, 1), sampled_img_feat           ## output BTS's decoder   ### img_feat_sampled.shape == (n_, n_pts, nv_, C_)
    ...
    def forward(self, xyz, coarse=True, viewdirs=None, far=False, only_density=False):   ## , infer=None):  ## ? "far"  ## Note: this forward propagation can be used for both training and eval
        ...
        with profiler.record_function("model_inference"):   ## create object with the name "model_inference". ## stop the timer when exiting the block
            ...
            sampled_features, invalid_features, sampled_img_feat = self.sample_features(xyz, use_single_featuremap=False)  # (sb, n_pts, n_v, 103)
            ...
            if self.nry: # and not infer:  ## only training with given viewdirs + sampled_features, otherwise we rely on features extract from RGB image
                ...
                img_feat = sampled_img_feat.reshape(-1, self.n_coarse, nmv, sampled_img_feat.shape[-1])       ### [n_rays, n_samples, n_views, n_feat]    ## Note: -1 == M / sb
...
#
#         if self.flip_augmentation and self.training:    do_flip = (torch.rand(1) > .5).item()
#         else:   do_flip = False
#
...
#             dim_size = None
#
#             # Run main NeRF network     ## decoder part
