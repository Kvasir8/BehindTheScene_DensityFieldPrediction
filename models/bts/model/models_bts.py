...
class MVBTSNet(torch.nn.Module):
    ...
    def compute_ray_diff(self, xyz: torch.Tensor, view_dirs: torch.Tensor) -> torch.Tensor:
        ...
        query and target ray directions, the last channel is the inner product of the two directions.
        """
        (B,n_pts,_,) = xyz.shape  ## Get the shape of the input point cloud and the feature grid (n, pts, spatial_coordinate == 3)
        ...
        xyz = torch.cat((xyz, ones), dim=-1)  # (B, 1, n_pts, 4) Concatenate the tensor of ones with the point cloud to create homogeneous coordinates
        xyz_projected = (self.grid_f_poses_w2c[:, :nv_, :3, :] @ xyz.permute(0,1,3,2)).permute(0,3,1,2)  # (B, n_pts, n_views, 3) Apply the camera poses to the point cloud to get the projected points and calculate the distance
        ...
        view_dirs_norm = (view_dirs / (torch.norm(view_dirs, dim=-1, keepdim=True) + 1e-6)).unsqueeze(-2)  # (B, n_pts, 1, 3)
