...
class MVBTSNet(torch.nn.Module):
    ...
    def encode(self, images, Ks, poses_c2w, ids_encoder=None, ids_render=None, images_alt=None, combine_ids=None):  ## ids_encoder:=which img to begin with
        ...
        img_feat = F.interpolate(img_feat_ms[-1], size=(h_, w_)).view(n_, nv_, img_feat_ms[-1].shape[1], h_, w_)    ## upsampling the feature maps from down-sampled 4 layers to the same spatial resolution of 1st layer
        ...
        self.img_feat = img_feat                    ## to feed into NeuRay's img feat
    ...
    def sample_features(self, xyz, use_single_featuremap=True):     ## 2nd arg: to control whether multiple feature maps should be combined into a single feature map or not. If True, the function will average the sampled features from multiple feature maps along the view dimension (nv) before returning the result. This can be useful when you want to combine information from multiple views or feature maps into a single representation.
        ...
        img_feat = self.img_feat[:, :nv_]                                             ## taking last layer of encoder to feed into NeuRay
        ...
        sampled_img_feat = F.grid_sample(img_feat.view(n_ * nv_, img_feat.shape[2], h_, w_), xy.view(n_ * nv_, 1, -1, 2), mode="bilinear", padding_mode="border", align_corners=False).view(n_, nv_, img_feat.shape[2], n_pts).permute(0, 3, 1, 2)
