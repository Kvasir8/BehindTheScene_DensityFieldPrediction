...
from models.bts.model.models_bts import MVBTSNet, BTSNet  ## BTSNet
...
class BTSWrapper(nn.Module):
    ...
    def compute_nvs_metrics(self, data):
        ...
        ssim_score = skimage.metrics.structural_similarity(
            imgs_pred_np, imgs_gt_np, multichannel=True, data_range=1, channel_axis=2
        )
        psnr_score = skimage.metrics.peak_signal_noise_ratio(
            imgs_pred_np, imgs_gt_np, data_range=1
        )
        lpips_score = self.lpips_vgg(imgs_pred, imgs_gt, normalize=False).mean()

        metrics_dict = {"ssim": ssim_score, "psnr": psnr_score, "lpips": lpips_score}
        return metrics_dict
